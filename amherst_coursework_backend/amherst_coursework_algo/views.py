...
def ai_course_advisor(request):
    ...
    try:
        ...
        all_courses = Course.objects.prefetch_related(
            "courseCodes", "departments", "keywords"
        ).all()
        relevant_courses = []
        seen_course_ids = set()
            ...
            formatted_recommendations.append(
                {
                    ...
                    "name": course.courseName,
                    "code": (
                        course.courseCodes.first().value
                        if course.courseCodes.exists()
                        else ""
                    ),
                    "credits": course.credits,
                    "reason": rec["reason"],
        ...
        return JsonResponse(
            {"success": False, "error": "Invalid cart data format"}, status=400
        )

    except Exception as e:
        logger.error(f"Error in AI advisor: {e}")
        return JsonResponse(
            {"success": False, "error": f"Failed to generate advice: {str(e)}"},
            status=500,
        )
