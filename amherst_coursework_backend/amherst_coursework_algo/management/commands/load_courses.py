...
from amherst_coursework_algo.models import Course, Department, CourseCode, OverGuidelines, Prerequisites, PrerequisiteSet, Professor, Section, Year
import json
                ...
                try:
                    ...
                    for i in range(len(deptList)):
                        dept, _ = Department.objects.get_or_create(
                            name=deptList[i],
                            defaults={
                                'code': deptList[i][:4].upper(), # Assume code is first 4 characters of name; will have to convert to some sort of lookup table later
                                'link': deptLinks[i]
                            }
                        )
                    ...
                    recommended = [
                        ...
                        for rec in course_data.get('prerequisites', {}).get('recommended', [])
                    ]
                    
                    required = []
                    for reqSet in course_data.get('prerequisites', {}).get('required', []):
                        prereq_set = PrerequisiteSet.objects.create()
                        courses = [Course.objects.get_or_create(id=req)[0] for req in reqSet]
                    ...
                    prereq, _ = Prerequisites.objects.get_or_create(
                            ...
                            professor_override = course_data.get('prerequisites', {}).get('profPermOver', False),
                    ...
                    overGuide, _ = OverGuidelines.objects.update_or_create(
                        ...
                        defaults={
                            ...
                            'prefForMajor': course_data.get('overGuidelines', {}).get('preferenceForMajor', False),
                            'overallCap': course_data.get('overGuidelines', {}).get('overallCap', 0),
                            'freshmanCap': course_data.get('overGuidelines', {}).get('freshmanCap', 0),
                            'sophomoreCap': course_data.get('overGuidelines', {}).get('sophomoreCap', 0),
                            'juniorCap': course_data.get('overGuidelines', {}).get('juniorCap', 0),
                            'seniorCap': course_data.get('overGuidelines', {}).get('seniorCap', 0),
                        }
                    )
                    ...
                    sections = []
                    for section_number, section_data in course_data.get('sections', {}).items():
                        sectionProfessor, _ = Professor.objects.get_or_create(name=section_data.get('profName', ''))
                    ...
                    self.stdout.write(
                        self.style.SUCCESS(f'Successfully created course "{course.courseName}"')
                    )
